...
# ---
# some modification, to use with routers (like cisco), witch require interective shell
# 
import warnings
import os
import pipes
...
class Connection(object):
    ...
    def __init__(self, runner, host, port, user, password, private_key_file, *args, **kwargs):
        ...
        self.close_chan = not self.interactive
        self.stout_timeout = 0.05
        self.stout_timeout_count = 20
        self.chan = None
        self.ssh = None
        self.sftp = None
        self.runner = runner
    ...
    def connect(self):
        ...
        if cache_key in SSH_CONNECTION_CACHE:
            ...
            self.chan = CHAN_CONNECTION_CACHE[cache_key]
        else:
            self.ssh, self.chan   = self._connect_uncached()
            SSH_CONNECTION_CACHE[cache_key] = self.ssh
            CHAN_CONNECTION_CACHE[cache_key] = self.chan
        return self
    ...
    def exec_command(self, cmd, tmp_path, become_user=None, sudoable=False, executable='/bin/sh', in_data=None):
        ...
        if not (self.runner.become and sudoable):
            ...
            if not self.interactive:
                ...
                if cmd == ';':
                    self.close_chan = True
                    return (0, 'close con', no_prompt_out + 'no stdout', no_prompt_out + 'no stderr')
                self.chan.send(cmd + '\n')
        ...
        if not self.interactive:
            ...
            while not self.chan.recv_ready():
                time.sleep(self.stout_timeout)
                timeout += 1
                if timeout >= self.stout_timeout_count:
                    msg = "Timeout expired, waiting respond on command: %s" % cmd
                    raise errors.AnsibleConnectionFailed(msg)

            stdout_len_1 = -1
            stdout_len_2 = 0
            stdout = ''
            while (stdout_len_1 != stdout_len_2) or  (self.chan.recv_ready()):
                stdout_len_1 = stdout_len_2
                if self.chan.recv_ready():
                    stdout += self.chan.recv(bufsize)
                stdout_len_2 = len(stdout)
                time.sleep(self.stout_timeout * 3 * timeout)
